...
{
    public class CreatePiezaDentalCommandHandler : BaseCommandHandler<CreatePiezaDentalCommand, CreatePiezaDentalResponse>
    {
        private PiezaDental _piezaDental { get; set; } = null!;

        public CreatePiezaDentalCommandHandler(IWorkContext workContext) : base(workContext)
        {
        }

        public override async Task<CreatePiezaDentalResponse> Handle(CreatePiezaDentalCommand command, CancellationToken cancellationToken)
        {
            var response = new CreatePiezaDentalResponse();

            _piezaDental = WorkContext.Services.Mapper.Map<PiezaDental>(command.PiezaDental);
            await em.CreateAsync(_piezaDental);
        ...
        {
            var response = e.Response as CreatePiezaDentalResponse;
